...
plot_predictions <- function(amodel){
  ...
  ggplot(newdata, aes(x = wave, y = tanh(z_prime), group = id)) + 
    ...
    geom_point(data = adt, aes(group = NULL, y = tanh(z)), alpha = .01, color = apal[[1]], position = position_jitter(), size = .5) +
    geom_point(color = apal[[3]], alpha = 1) + 
    geom_line(color = apal[[3]]) + 
    geom_line(aes(y = z_prime_id), color = apal[[4]], alpha = 1, size = .25) + 
...
for(i in 1:length(model_fits)){
  ...
  cat('\n\n### ', network_name, '{.tabset}\n\n') 
  cat('\n\n#### Plot\n\n')
  print(plot_predictions(model_fit) + plot_percents(proportion_variance_tables[[i]]) +
          plot_layout(ncol = 2, widths = c(4,1)))
  cat('\n\n#### Table\n\n')
  knitr::kable(tidyr::spread(proportion_variance_tables[[1]], out_of, percent), digits = 1)
